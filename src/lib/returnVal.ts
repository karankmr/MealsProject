...
export default class ReturnVal<T = any> {
  ...
  public static success<T>(data?: T, message?: string, httpCode?: number): ReturnVal<T> {
    ...
    return new ReturnVal<T>(true, message, data, httpCode);
  }
  public static error(message?: string, data?: any, httpCode?: number): ReturnVal {
    if (!message) {
  ...
  public static getErrorHttpCode(message): number {
    if ([EMessages.TWOFA_AUTHENTICATION_REQUIRED].includes(message)) {
      return 430;
    }
